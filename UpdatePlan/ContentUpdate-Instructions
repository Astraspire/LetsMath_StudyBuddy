# Lets Math v1 – Implementation Plan for Updated `Organized-Content-Library.md`

This document defines **goals, constraints, and concrete implementation steps** for integrating the updated content library and UI changes into the existing single‑page app.
...
## 1. Content Integration – Updated `Organized-Content-Library.md`
...
1. **Update or re‑run the existing content generator** (wherever you currently transform `Organized-Content-Library.md` → JS object).
   - Ensure:
...
<div id="quiz-results-details" class="quiz-results-collapsible collapsed">
  <div class="quiz-results-header">
    <button id="quiz-results-toggle" class="quiz-results-toggle">
      ...
      <span class="quiz-results-arrow">▸</span> <!-- rotate when expanded -->
    </button>
  </div>
...
- When **collapsed**, only the header (toggle) is visible, but styled big/eye‑catching.
...
2. **Add** rendering of the full results breakdown:
   ...
   - Build a per‑question report using `quizState`:
     - Question text.
     - All options.
     - User’s choice.
     - Correct answer.
...
## 3. Quiz History – Per-Item Deletion

### 3.1 Current Behavior

From README: quiz history uses `localStorage`, with “clear all” functionality and CSV export. [1]
...
Use a Unicode icon (`🗑`) or an inline SVG to avoid extra assets.
...
1. On `click` of `.quiz-history-delete`:
   ...
   - Remove the DOM element or re‑render list.

2. Keep existing “Clear All” behavior intact.

***

## 4. Review Topics – “Jump to Topic” Behavior
...
- “Jump to topic” dropdown on **Review tab** must show **only the four major sections**:
  - Algebra
  - Trigonometry
  - Calculus
  - Precalc General
- When a section is selected:
  - **Show only that section’s block** on the Review page.
...
1. **Change dropdown options**:
   - Limit options to the four major areas:
     ...
     - `calculus`
     - `precalc-general`
   - Labels: “Algebra Review”, “Trigonometry Review”, etc.
...
## 5. Make “Let’s Math” Title Navigate Home
...
- Clicking the app title **“Let’s Math”** should call `switchTab('home')`.
...
```html
<h1 id="app-title" class="app-title">Let’s Math</h1>
```
...
- [ ] “Clear All” still clears everything.
...
- [ ] Clicking “Let’s Math” always takes you to the Home tab.
...
- **No frameworks** – vanilla JS only. [1]
- Maintain current **CSS variable system** and color‑coding by area. [1]
- Keep all changes within the **single HTML file** layout (Study-BuddyApp.html).
- Do not change existing IDs or class names unless necessary; prefer adding new ones.

***

Once you have this wired up, we’ll be ready for the next iteration: tighter Yu-Gi-Oh! mode integration and possibly adding VR-friendly hooks.
